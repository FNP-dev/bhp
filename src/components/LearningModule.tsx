...
export function LearningModule({ completed }: Props) {
  ...
  const topicClass = (id: string) => `topic-${id.replace(/[^a-z0-9_-]/gi, "-")}`;
  const topicStyles = TOPICS.map(
    (t) => `
      .${topicClass(t.id)}-bg { background: ${t.color}; }
      .${topicClass(t.id)}-accent { background: color-mix(in oklab, ${t.color} 14%, transparent); color: ${t.color}; }
    `
  ).join("\n");
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {TOPICS.map((t, i) => {
              ...
              const topicIdClass = topicClass(t.id);
              return (
                <motion.div
                  key={t.id}
                ...
                >
                  ...
                  >
                    <div
                      className={`absolute -right-6 -top-6 h-32 w-32 rounded-full opacity-10 blur-2xl transition-opacity group-hover:opacity-25 ${topicIdClass}-bg`}
                    />
                    <div className="relative flex items-start justify-between mb-4">
                      <div className={`inline-flex h-12 w-12 items-center justify-center rounded-xl ${topicIdClass}-accent`}>
